    ...
    public class UsersRepository {
        ...
        */
        ...
        public async Task<User> GetUserAsync(string email, CancellationToken cancellationToken = default) {
            ...
            //    .FindAsync<User>(Builders<User>.Filter.Eq(f => f.Email, email));
            //return await _usersCollection.Find(Builders<User>.Filter.Eq(f => f.Email, email));
            return await _usersCollection.Find(f => f.Email == email).FirstOrDefaultAsync();
        }
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default) {
            try {
                ...
                var filter = Builders<Session>.Filter.Eq(f => f.UserId, storedUser.Id);
                var session = await _sessionsCollection.Find(filter).FirstOrDefaultAsync();
                // TODO Ticket: User Management
                ...
                filter,
                Builders<Session>.Update.Set(f => f.UserId, user.Email).Set(f => f.Jwt, user.AuthToken),
                new UpdateOptions { IsUpsert = session == null });
            ...
            try {
                ...
                */
                ...
                updateResult = await _usersCollection.UpdateOneAsync(
                    filter,
                    Builders<User>.Update.Set(f => f.Preferences, preferences),
