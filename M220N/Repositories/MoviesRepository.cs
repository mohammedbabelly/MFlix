...
namespace M220N.Repositories {
    public class MoviesRepository {
        ...
        public async Task<IReadOnlyList<MovieByCountryProjection>> GetMoviesByCountryAsync(
            ...
            //
            var res = await _moviesCollection
               .Find<Movie>(Builders<Movie>.Filter.AnyIn(f => f.Countries, countries))
               .Project<MovieByCountryProjection>(Builders<Movie>.Projection.Include(f => f.Title))
        ...
        public async Task<IReadOnlyList<Movie>> GetMoviesByGenreAsync(
            ...
            var res = await _moviesCollection
               .Find(Builders<Movie>.Filter.AnyIn(f => f.Genres, genres))
               .Limit(limit)
               .Skip(page * limit)
               .Sort(sort)
               .ToListAsync(cancellationToken);
