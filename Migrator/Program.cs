    ...
    class Program {
        ...
        static async Task Main(string[] args) {
            ...
            if (datePipelineResults.Count > 0) {
                ...
                //
                var res = new List<ReplaceOneModel<Movie>>();
                datePipelineResults.ForEach(c => res.Add(new ReplaceOneModel<Movie>(Builders<Movie>.Filter.Eq(f => f.Id, c.Id), c) { IsUpsert = true }));
                bulkWriteDatesResult = await _moviesCollection.BulkWriteAsync(res, new BulkWriteOptions() { IsOrdered = false });
            ...
            if (ratingPipelineResults.Count > 0) {
                ...
                var res = new List<ReplaceOneModel<Movie>>();
                ratingPipelineResults.ForEach(c => res.Add(new ReplaceOneModel<Movie>(Builders<Movie>.Filter.Eq(f => f.Id, c.Id), c) { IsUpsert = true }));
                bulkWriteRatingsResult = await _moviesCollection.BulkWriteAsync(res, new BulkWriteOptions() { IsOrdered = false });
